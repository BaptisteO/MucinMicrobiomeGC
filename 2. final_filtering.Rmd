...
#1. Checking the rarefication curves to assess reading depth
  ...
  samples falling below 1000 final reads are insufficiently sampled
...
##rarefaction filtering and minimum read setting
  Filtering the samples that don't achieve a plateau in the rarefication curves or fall below 1000 final reads
```{r}
...
<!-- Based on the positive control sequenced with the Ghent and Antwerp cohorts, a relative abundance threshold was set to filter out aberrant reads. -->

<!-- ```{r} -->
<!-- abundance_cutOff <- prune_samples(c("pos"), agglom_relAb) %>% -->
<!--     otu_table() %>% -->
<!--     as.data.frame() %>% -->
<!--     .[.>0] %>% -->
<!--     min() -->

<!-- otu_table(agglom_BA)[otu_table(agglom_relAb)<abundance_cutOff] <- 0 -->
<!-- otu_table(agglom_relAb)[otu_table(agglom_relAb)<abundance_cutOff] <- 0 -->
<!-- agglom_relAb <- filter_taxa(agglom_relAb, function(x) { sum(x) > 0 }, prune = TRUE) -->
<!-- agglom_BA <- filter_taxa(agglom_BA, function(x) { sum(x) > 0 }, prune = TRUE) -->
<!-- ``` -->
