...
##1.1 doing the co-occurance per tissue type control - adjacent - tumor
```{r}
...
#getting the presence absance data from the tumor samples
adundance_T <- subset_samples(agglom_relAb2, Tissue.Type == "tumor") %>%
  tax_glom(.,"Genus") %>%
  t(.)%>%
  NetworkPrepare(.)
#running the cooccurance for the tumor samples
Network_Tumor <- cooccur(mat = adundance_T,type = "spp_site",spp_names = TRUE)

#getting the presence absance data from the control samples
abundance_C <-subset_samples(agglom_relAb2, Tissue.Type == "noninflammed") %>%
  tax_glom(.,"Genus") %>%
  t(.) %>%
  NetworkPrepare(.)
#running the co-occurance for the control samples
...
##creating the visualistation of the co-occurance in the control samples
```{r}
...
##creating the visualistation of the co-occurance in the Tumor samples
```{r}
...
##Getting co-occurance per mucin phenotype
```{r}
###########Gastric phenotye#############################
...
```{r}
#getting all differentially abundant genera from the aldex objects:
...
# visNetwork(nodes = Gastric$nodes, edges = Gastric$edges) %>%
...
# visNetwork(nodes = Intestinal$nodes, edges = Intestinal$edges) %>%
#   visIgraphLayout(layout = "layout_in_circle")
...
# visNetwork(nodes = Mixed$nodes, edges = Mixed$edges) %>%
#   visIgraphLayout(layout = "layout_in_circle")
...
# visNetwork(nodes = Null$nodes, edges = Null$edges) %>%
#   visIgraphLayout(layout = "layout_in_circle")
...
```
##quantification of the network interactions
```{r}
...
residuals <- PropInteractions_Chi2$residuals%>%
  as.data.frame %>%
    rownames_to_column(var = "Genus")
writexl::write_xlsx(residuals,path = "Cooccur_genus_interactions_Chi2_residuals.xlsx")

residuals <- gather(residuals, key = Phenotype, value = residual,2:5)
#plotting the pearson residuals
ggplot(data = residuals, aes(x= Phenotype, y = Genus, fill = residual))+
  geom_tile(color = "white")+
  scale_fill_gradient2(low = "#30123B", high = "#7A0403", midpoint = 0, space = "Lab", name="Spearman residuals")+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, size = 12, hjust = 1),legend.position="bottom")


#subsetting the differential genera
residuals <- spread(residuals, key = Phenotype, value = residual)
Residuals_DifGenera <- subset(residuals,residuals$Genus %in% Differential_genera$Genus)

writexl::write_xlsx(Residuals_DifGenera,path = "Cooccur_diffgenera_interactions_Chi2_residuals.xlsx")
```
## running cooccurance for MUC13 high - mid - low
```{r}
###########Low MUC13#############################
#getting presence absance data from the tumor agglom_BAT
...
###########Mid MUC13#############################
#getting presence absance data from the tumor agglom_BAT
adundance_T_MUC13Mid <- subset_samples(agglom_BAT, Strat.Score_MUC13_T == 0) %>%
  t(.) %>%
  NetworkPrepare(.)

#running the cooccurance  
Network_MUC13Mid <- cooccur(mat =adundance_T_MUC13Mid,type = "spp_site",spp_names = TRUE)

MUC13Mid <- NetworkSig(Network_MUC13Mid)
MUC13Mid$nodes$shape <- NodeShape(MUC13Mid, Differential_MUC13)
MUC13Mid$nodes$color <- NodeColor(MUC13Mid, Differential_MUC13)
MUC13Mid$nodes$size <- NodeSize(MUC13Mid, Differential_MUC13)

MUC13Mid_Network <- network(MUC13Mid$edges, vertex.attr = MUC13Mid$nodes, matrix.type = "edgelist",ignore.eval = FALSE, directed = FALSE) %>%
  as_tbl_graph

MUC13Mid_NetworkGraph <- ggraph(MUC13Mid_Network,layout = "linear", circular=TRUE)+
  geom_edge_link(aes(linetype = dashes,colour = dashes))+
  geom_node_point(aes(shape = shape, colour = color),size = 3)+
  geom_node_text(aes(label = label, colour = color),size = 1,repel = TRUE,max.overlaps = 50)+
  theme_graph(plot_margin = margin(0.02,0.02,0.02,0.02,"cm"))+
  scale_color_manual(limits = c("High","Mid","Low"),values = c("#619CFF","#00BA38","#F8766D"))+
  scale_shape_manual(limits = c("MUC13","MUC2","MUC4","MUC5AC","MUC6","NA"), values = c(2,5,0,10,12,20))+
  ggtitle("A) Network of Mid MUC13 tumor tissues")
...
#quantification of the MUC13 networks
```{r}
...
residuals <- PropInteractions_Chi2$residuals%>%
  as.data.frame %>%
  rownames_to_column(.,var = "Genus")
Proportions_Interactions <- rownames_to_column(Proportions_Interactions, var = "Genus")

Genus_Chi2 <- full_join(Proportions_Interactions,residuals, by = "Genus" )

writexl::write_xlsx(residuals,path = "Cooccur_genusMUC13_interactions_Chi2_residuals.xlsx")
residuals <- gather(residuals, key = Phenotype, value = residual,2:5)
```
